...
"
#Eigen::ConjugateGradient< SparseMatrix<double>, Eigen::Lower，Eigen::IncompleteCholesky<double>> solver;
...
// [[Rcpp::export]]
Eigen::VectorXd solveCholesky(const Eigen::MappedSparseMatrix<double> &A,
                              const Eigen::VectorXd &b) {
    static Eigen::SimplicialLLT<Eigen::SparseMatrix<double>> solver;
        solver.analyzePattern(A);
        if (solver.info() != Eigen::Success) {
            Rcpp::stop("Symbolic decomposition failed");
        }
    solver.factorize(A);
    if (solver.info() != Eigen::Success) {
        Rcpp::stop("Numerical factorization failed");
    }
...
public:
    ...
    // Method to perform decomposition and solve Ax = b
    Eigen::VectorXd solve(const Eigen::MappedSparseMatrix<double> &A, 
                          const Eigen::VectorXd &b) {
        solver.analyzePattern(A); // Symbolic Decomposition
        if (solver.info() != Eigen::Success) {
            Rcpp::stop("Symbolic decomposition failed");
        }

        solver.factorize(A); // Numeric Decomposition
        if (solver.info() != Eigen::Success) {
            Rcpp::stop("Numerical factorization failed");
        }
...
RCPP_MODULE(CholeskySolverModule) {
    ...
    .constructor()
    .method("solve", &CholeskySolver::solve);
}
')